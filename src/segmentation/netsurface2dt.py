...
class NetSurf2dt:
    ...
    def get_surface_point( self, t, column_id ):
        ...
        k-=1
        x = self.centers[t][0] + self.col_vectors[column_id,0] * self.min_radius[0] + self.col_vectors[column_id,0] * (k-1)/float(self.K) * (self.max_radius[0]-self.min_radius[0])
        y = self.centers[t][1] + self.col_vectors[column_id,1] * self.min_radius[1] + self.col_vectors[column_id,1] * (k-1)/float(self.K) * (self.max_radius[1]-self.min_radius[1])
        return (x,y)
