...
class NetSurf2d:
    ...
    def get_surface_point( self, column_id ):
        ...
        k-=1
        x = self.center[0] + self.col_vectors[column_id,0] * self.min_radius[0] + self.col_vectors[column_id,0] * (k-1)/float(self.K) * (self.max_radius[0]-self.min_radius[0])
        y = self.center[1] + self.col_vectors[column_id,1] * self.min_radius[1] + self.col_vectors[column_id,1] * (k-1)/float(self.K) * (self.max_radius[1]-self.min_radius[1])
        return (x,y)
